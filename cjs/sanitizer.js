 ...
 */
export const sanitizeDocFragment = function _sanitizeDocFragment(config, input) {
  /* global DOMPurify */
...
export const _normalizeConfig = function _normalizeConfig(config) {
  ...
  let normalizedConfig = {}; // TODO https://github.com/mozilla/sanitizer-polyfill/issues/29
...
const _transformConfig = function transformConfig(config) {
  ...
  const dropAttrs = config.dropAttributes || []; // https://github.com/cure53/DOMPurify/issues/556
  ...
  const isdropElementsSet = dropElements !== DEFAULT_DROP_ELEMENTS && dropElements !== [];
  const isblockElementsSet = blockElements !== DEFAULT_BLOCKED_ELEMENTS && blockElements !== [];
  let domPurifyConfig = {
